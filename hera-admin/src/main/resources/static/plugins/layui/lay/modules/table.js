...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'laydate'], function (exports) {
    ...
    Class.prototype.events = function () {
        ...
        that.layBody.on('change', '.' + ELEM_EDIT, function () {
            ...
            if (othis.parent().data('edit') != 'text') {
                return;
            }
            that.eachCols(function (i, item) {
        ...
        that.layBody.on('click', 'td', function () {
            ...
            if (editType == 'text') {
                ...
                //append实现：如果有就focus 没有就增加
                othis.find('.' + ELEM_EDIT)[0] || othis.append(input);
                input.focus();
                return;
            }
            if (editType == 'datetime') {
                var input = $('<input type="text" class="layui-input ' + ELEM_EDIT + '">');
                othis.find('.' + ELEM_EDIT)[0] || othis.append(input);
                //append实现：如果有就focus 没有就增加
                console.log(othis.data('content') || elemCell.text())
                laydate.render({
                    type: 'datetime',
                    format: 'yyyy-MM-dd HH:mm:ss',
                    value: othis.data('content') || elemCell.text(),
                    isInitValue: true,
                    calendar: true,
                    mark: {
                        '0-0-10': '工资'
                    ...
                    ready: function(data) {
                        ...
                        othis.siblings(ELEM_CELL).html(function (value) {
                            return templet ? function () {
                                return typeof templet === 'function'
                                    ? templet(data)
                                    : laytpl($(templet).html() || val).render(data)
                            }() : value;
                        }(val));
                        othis.parent().data('content', val);
                        othis.remove();
                        data[field] = val; //更新缓存中的值
                        layui.event.call(input[0], MOD_NAME, 'edit(' + filter + ')',  { data:data,
                            value: val
                            , field: field
                        });
                    }
